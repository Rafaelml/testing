...
public class DiaBreak {
	        ...
	        if(validaHora(hora) && huecoLibre(hora, cita.getDuracion())) {
	            ...
	            if(citas[horaIni] == null) {
	                ...
	                for(int i = 0; i < duracion; i++) {
	                	if(horaIni + duracion < citas.length + 1){//Comprobamos que después de las cinco no existan citas de más de una hora 
	    ...
	    public boolean huecoLibre(int hora, int duracion) {
	    	if(!validaHora(hora) || !validaHora(hora+duracion-1))
	    		return false;
	    	int horaIni = hora - PRIMERA_CITA;
            if(citas[horaIni] == null) {
            	duracion--;
